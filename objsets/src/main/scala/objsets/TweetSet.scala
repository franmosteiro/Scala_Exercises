...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = filterAcc((x => contains(x) || that.contains(x)), that)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(max: Tweet): Tweet =
    right.mostRetweetedAcc(left.mostRetweetedAcc(if (elem.retweets > max.retweets) elem else max))
